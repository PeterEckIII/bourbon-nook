...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create whiskey-nook-f085
  fly apps create whiskey-nook-f085-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app whiskey-nook-f085
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app whiskey-nook-f085-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'whiskey-nook-f085-staging' does not match app name in config file 'whiskey-nook-f085'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name whiskey-nook-f085-db
  fly postgres attach --app whiskey-nook-f085 whiskey-nook-f085-db

  fly postgres create --name whiskey-nook-f085-staging-db
  fly postgres attach --app whiskey-nook-f085-staging whiskey-nook-f085-staging-db
  ```
